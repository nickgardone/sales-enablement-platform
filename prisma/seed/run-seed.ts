...
import { createRng, SEED_NOW } from "./rng";
import {
  REGIONS,
  OEM_BRANDS,
...
// `let` (not `const`) so runSeed() can hand out a fresh generator on every
// call — needed because this runs inside a long-lived Next.js process too
// (the admin console's "reset demo data" button), not just a fresh CLI
// process each time, and reseeding must stay deterministic per call.
let rng = createRng();
...
export async function runSeed() {
  // Fresh generator + cleared event buffers each call — see the `let rng`
  // comment above for why this can't just be module-level const state.
  rng = createRng();
  signals.length = 0;
  ...
  for (let m = 0; m < 3; m++) {
    const period = new Date(SEED_NOW);
    period.setDate(1);
  ...
  for (const rooftop of leadSourceRooftops) {
    ...
    const isBreached = slaDueAt < SEED_NOW && rng.bool(0.15);
